...
public class Gui extends Application {
    ...
    private static final List<Number> YEARS = IntStream.rangeClosed(1900, Year.now().getValue())
    ...
    private static Text createTextFor(Property<?> property) {
        Text text = new Text();
        property.addListener((_, _, newValue) -> text.setText(newValue.toString()));
        return text;
    }
    ...
    private ObjectProperty<MediaPlayer> createMediaPlayer(ChartsEntryProperty chartsEntry) {
        ...
        chartsEntry.audio.addListener((_, _, newAudio) -> {
            MediaPlayer player = new MediaPlayer(new Media(newAudio));
            player.errorProperty().addListener((_, _, newValue) -> newValue.printStackTrace());
            player.setAutoPlay(true);
            player.volumeProperty().bind(volume);
            player.setOnEndOfMedia(() -> {
                // Stopping the player resets playback to start time, but it doesn't reset
        ...
        mediaPlayer.addListener((_, oldPlayer, _) -> {
            if (oldPlayer != null) {
                oldPlayer.dispose();
            }
        });
    ...
    private Button createShowSolutionButton() {
        ...
        showSolutionButton.setOnAction(_ -> showSolution.set(!showSolution.get()));
        showSolution.addListener((_, _, newState) -> showSolutionButton.setText(newState ? "Hide Solution" : "Show Solution"));
        return showSolutionButton;
    }

    private Button createPlayButton(ObjectProperty<MediaPlayer> mediaPlayer) {
        Button playButton = new Button();
        playButton.setOnAction(_ -> togglePlayPause(mediaPlayer.get()));
        playerState.addListener((_, _, newState) -> {
    ...
    private ProgressBar createProgressBar(ObjectProperty<MediaPlayer> mediaPlayer) {
        ...
        mediaPlayer.addListener((_, _, newPlayer) -> {
            progressBar.setProgress(0.0);
            newPlayer.currentTimeProperty().addListener(_ -> {
    ...
    private Node createSolution(ChartsEntryProperty chartsEntry) {
        ...
        showSolution.addListener((_, _, newState) -> {
            if (newState) {
    ...
    private ImageViewPane createCoverImage(StringProperty cover) {
        ...
        cover.addListener((_, _, newValue) -> {
            try {
    ...
    private TextFlow createSolutionText(ChartsEntryProperty chartsEntry) {
        ...
        moreDetails.setOnAction(_ -> getHostServices().showDocument(chartsEntry.moreDetails.get()));
